...
extension TabBarManager: TabBarManagerProtocol {
	...
	var selectedTab: Tab {
		guard tabBar.selectedIndex > 1 else {
			assertionFailure("Ошибка получения выбранного индекса в таббаре(")
			return .home
		}

		return Tab(rawValue: tabBar.selectedIndex) ?? .home
	}
}
...
extension TabBarManager: UITabBarControllerDelegate, UITabBarDelegate {
	func tabBarController(_ tabBarController: UITabBarController, didSelect viewController: UIViewController) {
		...
		if selectedIndex == previousSelectedIndex {
			delegate?.repeatedTap(tab: Tab(rawValue: selectedIndex) ?? .home)
		}
		delegate?.didSelectTab(tab: Tab(rawValue: selectedIndex) ?? .home)
